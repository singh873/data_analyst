...
GEMINI_API_URL = "https://aipipe.org/geminiv1beta/models/gemini-2.5-flash:generateContent"
horizon_api = os.getenv("horizon_api")

# Read three Aipipe keys and build a rotation list. Set these in your .env as:
# AIPIPE_API_KEY_1, AIPIPE_API_KEY_2, AIPIPE_API_KEY_3
AIPIPE_API_KEY_1 = os.getenv('AIPIPE_API_KEY_1')
AIPIPE_API_KEY_2 = os.getenv('AIPIPE_API_KEY_2')
AIPIPE_API_KEY_3 = os.getenv('AIPIPE_API_KEY_3')
AIPIPE_API_KEYS = [k for k in (AIPIPE_API_KEY_1, AIPIPE_API_KEY_2, AIPIPE_API_KEY_3) if k]

def _get_aipipe_key_for_try(try_index: int):
    """Return an Aipipe key for this retry index, rotating across configured keys."""
    if not AIPIPE_API_KEYS:
        return None
    return AIPIPE_API_KEYS[try_index % len(AIPIPE_API_KEYS)]
...
async def ping_gemini(question_text, relevant_context="", max_tries=3):
    """Call the Aipipe Gemini endpoint rotating between configured AIPIPE_API_KEY_* keys per retry.

    Each retry will use the next key in the rotation so load is distributed.
    """
    tries = 0
    while tries < max_tries:
        try:
            print(f"gemini (aipipe) is running attempt {tries + 1}/{max_tries}")
            effective_api_key = _get_aipipe_key_for_try(tries)
            if not effective_api_key:
                print("\u274c No AIPIPE API keys configured (AIPIPE_API_KEY_1/2/3)")
                return {"error": "AIPIPE API key(s) not configured"}
            ...
            print(f"Error during Aipipe Gemini call (attempt {tries + 1}): {e}")
            tries += 1
            if tries < max_tries:
                print(f"Retrying... ({max_tries - tries} attempts remaining)")
            else:
                print(f"All {max_tries} attempts failed for Aipipe Gemini")
    return {"error": "Aipipe Gemini failed after max retries"}
...
async def ping_gemini_pro(question_text, relevant_context="", max_tries=3):
    """Call the Aipipe Gemini endpoint (pro/code) rotating between configured keys per retry."""
    tries = 0
    while tries < max_tries:
        try:
            print(f"gemini pro (aipipe) attempt {tries + 1}/{max_tries}")
            effective_api_key = _get_aipipe_key_for_try(tries)
            if not effective_api_key:
                print("\u274c No AIPIPE API keys configured (AIPIPE_API_KEY_1/2/3)")
                return {"error": "AIPIPE API key(s) not configured"}
            ...
            async with httpx.AsyncClient(timeout=240) as client:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        print(f"Unexpected Gemini Pro response structure: {e}")
                        raise
                else:
                    print(f"Aipipe Gemini Pro API error: {response.status_code} - {response.text}")
                    if response.status_code >= 500:
                        raise Exception(f"Server error {response.status_code}: {response.text}")
                    else:
                        return {"error": f"Client error {response.status_code}: {response.text}"}
        except Exception as e:
            print(f"Error in Aipipe Gemini Pro API call (attempt {tries + 1}): {e}")
            tries += 1
            if tries < max_tries:
                print(f"Retrying... ({max_tries - tries} attempts remaining)")
            else:
                print(f"All {max_tries} attempts failed for Aipipe Gemini Pro")
...
async def extract_data_with_gemini_pro(base64_image: str, filename: str, max_tries: int = 3) -> str:
    """Use Aipipe Gemini Pro to extract text/data from images, rotating keys per retry.
    ...
    while tries < max_tries:
        try:
            print(f"🤖 Using Aipipe Gemini Pro to analyze image: {filename} (attempt {tries+1}/{max_tries})")
            effective_api_key = _get_aipipe_key_for_try(tries)
            if not effective_api_key:
                print("⚠️ No AIPIPE API keys configured (AIPIPE_API_KEY_1/2/3)")
                return None
                    ...
                    if "candidates" in gemini_response and len(gemini_response["candidates"]) > 0:
                        ...
                        if "content" in candidate and "parts" in candidate["content"]:
                            ...
                            if extracted_text and extracted_text.strip() and "NO_CONTENT_FOUND" not in extracted_text:
                                print(f"✅ Aipipe Gemini Pro successfully analyzed image: {filename}")
                                return extracted_text.strip()
                            else:
                                print(f"ℹ️ Aipipe Gemini Pro returned no meaningful content for: {filename}")
                                return None
                        else:
                            print("⚠️ Aipipe Gemini Pro response missing expected content structure")
                            raise Exception("Invalid response structure")
                    else:
                        print("⚠️ Aipipe Gemini Pro response missing candidates")
            ...
            print(f"❌ Error in Aipipe Gemini Pro image analysis (attempt {tries+1}): {e}")
            tries += 1
            if tries < max_tries:
                await asyncio.sleep(1 + tries)
            else:
                print(f"❌ All {max_tries} attempts failed for Aipipe Gemini Pro on image: {filename}")
                return None
